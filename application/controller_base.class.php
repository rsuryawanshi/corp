...
Abstract Class baseController {
    ...
    */
    ...
    public $strRssUrl 		= 'static/feed/rss.xml';
    public $strSitemapUrl 	= 'static/feed/sitemap.xml';
    
    public $arrMixActivityStatus = array(
        1 => array( 'POSTQUOTE' 	=> "Posted a requirement" )
    );
        
	public $is_mobile = false;

	private $arrHWUsers 	= array( 'rbsuryawanshiindia@gmail.com', 'chiragkumar.shanker@gmail.com' );
	public $boolIsHWUser 	= false;

    function __construct( $registry ) {
    	
        $this->registry = $registry;
        
        $this->registry->template->username = '';
		$this->registry->template->userid = '';

		$this->registry->template->iseluser = false;
		
		//get user name
        if( $this->isLoggedin() ) {
        	
            $user_arr 	= $_COOKIE['token'];
            $hash 		= $this->decrypt( ( $user_arr ), 'kiranrahul%$');
            $user_arr 	= unserialize( $hash );

            if(isset( $user_arr['user_name'] ) ) {
                $this->username = ucwords( $user_arr['user_name'] );
            }
        }
        
        $this->registry->template->base_url 				= $this->registry->serverurl;
        $this->registry->template->static_url 				= $this->registry->serverurl;
        $this->registry->template->rss_url 					= $this->registry->serverurl . 'static/feed/rss.xml';
        $this->registry->template->sitemap_url 				= $this->registry->serverurl . 'static/feed/sitemap.xml';
        
		$this->registry->template->local = 'India';		
		$this->registry->template->arrMixActivityStatus 		= $this->arrMixActivityStatus;		
    ...
    function isLoggedin() {
    	...
    	if( isset( $_COOKIE['userid'] ) && isset( $_COOKIE['token'] ) ) {
            ...
            //$redirecturl = $this->registry->serverurl.'user/login';
    ...
    function checkLogin( $email, $pass ) {
    	...
    	if( $email && $pass ) {
            ...
            $pdata = $api->MakeCurlApi($Data);
            
            if (isset($pdata->success)) {
    ...
    public function MakeCurlApi( $userData = array(), $flag = '1' ) {
    ...
    public function getUri( $i = 0 ) {

    	if( 0 != $i ) {
            if( isset( $_GET['rt'] ) ) {
            	
                $uri_arr = explode('/', $_GET['rt']);
    ...
    public function custmiseString($str) {
    ...
    public function checkifmobile($userAgent) {

    	if (eregi("NokiaN96(.*)", $userAgent, $regs) || eregi("NokiaN95(.*)", $userAgent, $regs)) {
            $tmp_arr_agents['otype'] = "iphone";
            $tmp_arr_agents['mobiletype'] = "N96";
            $tmp_arr_agents['css_name'] = "N96.css";
            return $tmp_arr_agents;
        } elseif (eregi("iPhone(.*)", $userAgent, $regs) || eregi("nokian97(.*)", $userAgent, $regs) || eregi("Android(.*)", $userAgent, $regs) || eregi("SAMSUNG-GT-S5233(.*)", $userAgent, $regs) || eregi("HTC_P3450(.*)", $userAgent, $regs)) {
            $tmp_arr_agents['otype'] = "iphone";
            $tmp_arr_agents['mobiletype'] = "iphone";
            $tmp_arr_agents['css_name'] = "iphone.css";
            return $tmp_arr_agents;
        } elseif (eregi("SEC-SGHX400(.*)", $userAgent, $regs) || eregi("Nokia3100(.*)", $userAgent, $regs) || eregi("Nokia3220(.*)", $userAgent, $regs) || eregi("SonyEricssonK500i(.*)", $userAgent, $regs) || eregi("SEC-SGHC230C(.*)", $userAgent, $regs) || eregi("SonyEricssonK700i(.*)", $userAgent, $regs) || eregi("Nokia3120(.*)", $userAgent, $regs) || eregi("MOT-C261(.*)", $userAgent, $regs) || eregi("SonyEricssonJ220i(.*)", $userAgent, $regs) || eregi("SonyEricssonJ230i(.*)", $userAgent, $regs) || eregi("Nokia6070(.*)", $userAgent, $regs) || eregi("SonyEricssonW200i(.*)", $userAgent, $regs) || eregi("LG-KG200j(.*)", $userAgent, $regs)) {
            $tmp_arr_agents['otype'] = "mobile";
            $tmp_arr_agents['mobiletype'] = "mobile";
            $tmp_arr_agents['css_name'] = "128.css";
            return $tmp_arr_agents;
        } elseif (eregi("Nokia6600(.*)", $userAgent, $regs) || eregi("MOT-V600(.*)", $userAgent, $regs) || eregi("mot-V3(.*)", $userAgent, $regs) || eregi("SonyEricssonW800i(.*)", $userAgent, $regs) || eregi("NokiaN91(.*)", $userAgent, $regs) || eregi("Nokia3250(.*)", $userAgent, $regs) || eregi("SonyEricssonK750i(.*)", $userAgent, $regs) || eregi("SonyEricssonW550i(.*)", $userAgent, $regs) || eregi("MOT-L9(.*)", $userAgent, $regs) || eregi("SonyEricssonZ710i(.*)", $userAgent, $regs) || eregi("SonyEricssonK550i(.*)", $userAgent, $regs) || eregi("SonyEricssonW700i(.*)", $userAgent, $regs) || eregi("SonyEricssonK530i(.*)", $userAgent, $regs) || eregi("NokiaN72(.*)", $userAgent, $regs) || eregi("NokiaN70(.*)", $userAgent, $regs) || eregi("Nokia6681(.*)", $userAgent, $regs) || eregi("Nokia3230(.*)", $userAgent, $regs) || eregi("Nokia6630(.*)", $userAgent, $regs) || eregi("Nokia3660(.*)", $userAgent, $regs) || eregi("NokiaN-GageQD(.*)", $userAgent, $regs) || eregi("Nokia7610(.*)", $userAgent, $regs) || eregi("SEC-SGHE530C(.*)", $userAgent, $regs)) {
            $tmp_arr_agents['otype'] = "mobile";
            $tmp_arr_agents['mobiletype'] = "mobile";
            $tmp_arr_agents['css_name'] = "176.css";
            return $tmp_arr_agents;
        } elseif (eregi("Nokia7370(.*)", $userAgent, $regs) || eregi("MOT-A780(.*)", $userAgent, $regs) || eregi("MOT-A768i(.*)", $userAgent, $regs) || eregi("NokiaE60(.*)", $userAgent, $regs) || eregi("SonyEricssonP990i(.*)", $userAgent, $regs) || eregi("NokiaN93(.*)", $userAgent, $regs) || eregi("Nokia6280(.*)", $userAgent, $regs) || eregi("SEC-SGHD840(.*)", $userAgent, $regs) || eregi("Nokia5300(.*)", $userAgent, $regs) || eregi("Nokia6300(.*)", $userAgent, $regs) || eregi("Nokia6233(.*)", $userAgent, $regs) || eregi("NokiaE50(.*)", $userAgent, $regs) || eregi("NokiaN73(.*)", $userAgent, $regs) || eregi("NokiaE65(.*)", $userAgent, $regs) || eregi("SonyEricssonW830i(.*)", $userAgent, $regs) || eregi("SonyEricssonW580i(.*)", $userAgent, $regs) || eregi("Nokia5310(.*)", $userAgent, $regs) || eregi("Nokia6500c(.*)", $userAgent, $regs) || eregi("SonyEricssonP900(.*)", $userAgent, $regs) || eregi("SonyEricssonK790i(.*)", $userAgent, $regs) || eregi("SGH-D900i(.*)", $userAgent, $regs) || eregi("SonyEricssonW880i(.*)", $userAgent, $regs) || eregi("SonyEricssonK770i(.*)", $userAgent, $regs) || eregi("SonyEricssonW960i(.*)", $userAgent, $regs) || eregi("SonyEricssonp1i(.*)", $userAgent, $regs) || eregi("SonyEricssonk660i(.*)", $userAgent, $regs) || eregi("NokiaE71(.*)", $userAgent, $regs) || eregi("NokiaE66(.*)", $userAgent, $regs) || eregi("MOT-RAZRV6(.*)", $userAgent, $regs) || eregi("NokiaN81(.*)", $userAgent, $regs) || eregi("NokiaE51(.*)", $userAgent, $regs) || eregi("NokiaE61(.*)", $userAgent, $regs) || eregi("NokiaN76(.*)", $userAgent, $regs) || eregi("BlackBerry(.*)", $userAgent, $regs)) {
            $tmp_arr_agents['otype'] = "mobile";
            $tmp_arr_agents['mobiletype'] = "mobile";
            $tmp_arr_agents['css_name'] = "240.css";
            return $tmp_arr_agents;
        } elseif (eregi("Nokia9500(.*)", $userAgent, $regs) || eregi("Nokia9300(.*)", $userAgent, $regs) || eregi("NokiaN90(.*)", $userAgent, $regs) || eregi("NokiaN80(.*)", $userAgent, $regs) || eregi("NokiaN71(.*)", $userAgent, $regs) || eregi("SamsungP310(.*)", $userAgent, $regs) || eregi("Nokia7710(.*)", $userAgent, $regs) || eregi("NokiaE90(.*)", $userAgent, $regs)) {
            $tmp_arr_agents['otype'] = "mobile";
            $tmp_arr_agents['mobiletype'] = "mobile";
            $tmp_arr_agents['css_name'] = "320.css";
            return $tmp_arr_agents;
        } elseif (eregi("nokia(.*)", $userAgent, $regs) || eregi("alcatel(.*)", $userAgent, $regs) || eregi("benq(.*)", $userAgent, $regs) || eregi("benqsie(.*)", $userAgent, $regs) || eregi("bleu(.*)", $userAgent, $regs) || eregi("docomo(.*)", $userAgent, $regs) || eregi("ericsson(.*)", $userAgent, $regs) || eregi("Fly(.*)", $userAgent, $regs) || eregi("haier(.*)", $userAgent, $regs) || eregi("hitachi(.*)", $userAgent, $regs) || eregi("htc(.*)", $userAgent, $regs) || eregi("huawei(.*)", $userAgent, $regs) || eregi("imate(.*)", $userAgent, $regs) || eregi("lg(.*)", $userAgent, $regs) || eregi("mot(.*)", $userAgent, $regs) || eregi("portalmmm(.*)", $userAgent, $regs) || eregi("palm(.*)", $userAgent, $regs) || eregi("philips(.*)", $userAgent, $regs) || eregi("sagem(.*)", $userAgent, $regs) || eregi("Samsung(.*)", $userAgent, $regs) || eregi("sch(.*)", $userAgent, $regs) || eregi("sharp(.*)", $userAgent, $regs) || eregi("sony(.*)", $userAgent, $regs) || eregi("tmobile(.*)", $userAgent, $regs) || eregi("vm4050(.*)", $userAgent, $regs) || eregi("acer_pro80(.*)", $userAgent, $regs)) {
            $tmp_arr_agents['otype'] = "mobile";
            $tmp_arr_agents['mobiletype'] = "mobile";
            $tmp_arr_agents['css_name'] = "176.css";
            return $tmp_arr_agents;
        } elseif (eregi("Opera Mini(.*)", $userAgent, $regs) || eregi("Opera Mobi(.*)", $userAgent, $regs)) {
            $tmp_arr_agents['otype'] = "mobile";
            $tmp_arr_agents['mobiletype'] = "mobile";
            $tmp_arr_agents['css_name'] = "176.css";
            return $tmp_arr_agents;
        } elseif (eregi("LEMON(.*)", $userAgent, $regs) || eregi("Micromax(.*)", $userAgent, $regs) || eregi("WAP(.*)", $userAgent, $regs) || eregi("Karbonn(.*)", $userAgent, $regs)) {
            $tmp_arr_agents['otype'] = "mobile";
            $tmp_arr_agents['mobiletype'] = "mobile";
            $tmp_arr_agents['css_name'] = "176.css";
            return $tmp_arr_agents;
    ...
    protected function spamfilter( $content ) {
    	
        if( $content ) {
            $url = 'http://'.$this->_apiIP.'/spam/spam.php?word='.urlencode($content).'';
            $ch = curl_init($url);
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
            curl_setopt($ch, CURLOPT_HEADER, 0);  // DO NOT RETURN HTTP HEADERS
            ...
            $rec_data = curl_exec($ch);
            curl_close($ch);
            return preg_replace("'\s+'", ' ', $rec_data);
        }
    }
...
//		print $user_agent;
		...
		$iphone = $ipad = $android = $opera = $blackberry = $palm = $windows = false;

		$mobile_browser   = true; // set mobile browser as false till we can prove otherwise
		$user_agent       = $_SERVER['HTTP_USER_AGENT']; // get the user agent value - this should be cleaned to ensure no nefarious input gets executed
		$accept           = $_SERVER['HTTP_ACCEPT']; // get the content accept value - this should be cleaned to ensure no nefarious input gets executed
...
//		  $mobile_browser = $ipad; // mobile browser is either true or false depending on the setting of ipad when calling the function
		  $mobile_browser = true; // mobile browser is either true or false depending on the setting of ipad when calling the function
